...
interface VerAtaOrientadorProps {
  params: Promise<{ id: string }>;
}

export default async function OrientadorVerAtaPage({ params }: VerAtaOrientadorProps) {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {/* Voltar */}
      <Link
        href={`/orientador/alunos/${reuniao.projeto.orientandoId}`}
      ...
      <div className="border-b border-slate-900/60 pb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-extrabold tracking-tight flex items-center gap-2">
            <FileText className="h-8 w-8 text-indigo-400 shrink-0 animate-pulse" />
            Ata de Orientação #{reuniao.numeroEncontro}
          </h1>
          <p className="text-slate-400 text-sm">
            Orientando: <span className="font-semibold text-slate-300">{reuniao.projeto.orientando.nome}</span> | Projeto: {reuniao.projeto.titulo}
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs font-semibold px-3 py-1.5 bg-slate-900 border border-slate-800 text-slate-300 rounded-xl">
          ...
          Registrado em {new Date(reuniao.dataHoraInicio).toLocaleString('pt-BR')}
        </div>
      </div>
      ...
      <div className="space-y-10">
        ...
        <div className="glass p-6 rounded-2xl border border-slate-900/60 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-sm">
            ...
            <div className="space-y-0.5">
              <span className="text-xs text-slate-500 block">Situação do Cronograma</span>
              <span className={`inline-flex items-center gap-1.5 font-bold text-xs ${
                ...
                {reuniao.situacaoCronograma === 'VERDE' ? '🟢 Verde (Em dia)' : reuniao.situacaoCronograma === 'AMARELO' ? '🟡 Amarelo (Atenção)' : '🔴 Vermelho (Crítico)'}
              </span>
            </div>
          </div>
        </div>
