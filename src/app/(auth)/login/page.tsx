...
function LoginContent() {
  ...
  React.useEffect(() => {
    if (status === 'authenticated' && !error && !isTransitioning) {
      router.push('/');
    }
  }, [status, router, error, isTransitioning]);
  ...
  const handleDevLogin = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      try {
        const result = await signIn('credentials', { email: emailDev, redirect: false });
        if (result?.error) {
          setLoadingDev(false);
  ...
  return (
    <>
      {/* Overlay de Transição Animada do Login */}
      {isTransitioning && (
        <div className="fixed inset-0 bg-[#030712] z-[999] flex items-center justify-center animate-in fade-in duration-500">
          <div 
            className={`transition-all duration-[850ms] ease-in-out flex flex-col items-center justify-center space-y-4 ${
              logoSlide 
                ? 'scale-25 translate-x-[calc(-50vw+120px)] translate-y-[calc(-50vh+32px)] opacity-80' 
                : 'scale-100'
            }`}
          >
            <div className="relative w-72 h-32 rounded-3xl overflow-hidden border border-blue-500/25 shadow-2xl shadow-blue-600/10 bg-[#0b0f19]">
              <video
                autoPlay
                loop
                muted
              ...
              >
                <source src="/marca_soia_animada.mp4" type="video/mp4" />
              </video>
            </div>
            {!logoSlide && (
              <p className="text-xs font-bold uppercase tracking-widest text-blue-400 animate-pulse">
      ...
      <div className="relative z-10 w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-center">
        ...
        <div className="lg:col-span-7 space-y-8 text-left animate-in slide-in-from-left duration-500">
          <div className="space-y-4">
            {/* Logo SOIA com Vídeo Fixo (Marca Animada 3D) */}
            <div className="relative w-44 h-16 rounded-2xl overflow-hidden border border-blue-500/15 shadow-lg shadow-blue-500/5 bg-[#0b0f19]/40 backdrop-blur-md">
              <video
                autoPlay
                loop
                muted
        ...
        <div className="lg:col-span-5 flex justify-center animate-in slide-in-from-right duration-500">
          <div className="w-full max-w-md px-6 py-10 glass rounded-3xl shadow-2xl relative">
            ...
            <div className="flex flex-col items-center text-center space-y-4 mb-6">
              {/* Logo SOIA Pequena em Vídeo para cabeçalho do Form */}
              <div className="relative w-28 h-10 rounded-xl overflow-hidden border border-blue-500/15 bg-[#0b0f19]/60">
                <video
                  autoPlay
                  loop
                  muted
            ...
            <div className="space-y-5">
              <div className="space-y-1.5">
                ...
                <button
                  ...
                  disabled={status === 'loading' || loadingDev || isTransitioning}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span>{status === 'loading' || isTransitioning ? 'Conectando...' : 'Entrar com o Google'}</span>
                  </div>
              ...
              <form onSubmit={handleDevLogin} className="space-y-3">
                ...
                <button
                  type="submit"
                  disabled={status === 'loading' || loadingDev || !emailDev || isTransitioning}
