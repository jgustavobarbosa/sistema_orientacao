  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setErro(text || 'Ocorreu um erro ao processar sua solicitação.');
      } else {
        setSucesso(true);
      }
    } catch (err) {
  ...
  if (!token) {
    return (
      <div className="text-center py-8 space-y-4">
        ...
        <h3 className="text-lg font-bold text-slate-200">Parâmetro Inválido</h3>
        <p className="text-xs text-slate-400 max-w-sm mx-auto">
          O link de redefinição de senha está quebrado ou incompleto. Por favor, solicite um novo link.
        </p>
        <Link 
          href="/forgot-password" 
          className="inline-block py-2 px-4 bg-slate-900 border border-slate-800 text-slate-300 text-xs rounded-xl hover:bg-slate-850"
        >
          Solicitar Novo Link
        </Link>
      </div>
    ...
    <div className="w-full">
      {erro && (
        <div className="flex items-start gap-3 p-4 mb-5 bg-red-500/10 border border-red-500/20 rounded-xl text-red-200 text-xs animate-in fade-in duration-200">
          ...
          <div>
            <h4 className="font-semibold text-red-400">Falha ao redefinir</h4>
      ...
      {sucesso ? (
        <div className="space-y-6 text-center py-4 animate-in fade-in duration-350">
          ...
          <div className="space-y-2">
            <h3 className="text-lg font-bold text-slate-200">Senha Redefinida!</h3>
            <p className="text-xs text-slate-400 leading-relaxed">
              Sua nova senha de acesso foi salva e criptografada com sucesso. Você já pode se autenticar na plataforma.
            </p>
          </div>
          <Link 
            href="/login" 
        ...
        <div className="flex flex-col items-center text-center space-y-4 mb-6">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1">
              Escolha uma senha forte para proteção do seu espaço científico
            </p>
          </div>
        </div>
